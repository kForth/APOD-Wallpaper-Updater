...
def get_desktop_environment() -> str:
    ...
    # From https://github.com/1j01/textual-paint/blob/main/src/textual_paint/wallpaper.py
    # which takes from:
    # https://stackoverflow.com/a/21213358/2624876
    # which takes from:
    ...
    if desktop_session := os.environ.get("DESKTOP_SESSION"):
        if desktop_session.lower() in [
            "gnome",
            "unity",
            "cinnamon",
            "mate",
        ...
        # There is no guarantee that they won't do the same with the other desktop envs.
        if "xfce" in desktop_session or desktop_session.startswith("xubuntu"):
            return "xfce4"
        if desktop_session.startswith("ubuntustudio"):
            return "kde"
    ...
    if os.environ.get("KDE_FULL_SESSION") == "true":
        return "kde"
    if (_id := os.environ.get("GNOME_DESKTOP_SESSION_ID")) and ("deprecated" not in _id):
        return "gnome2"
...
def is_running(process: str) -> bool:
    ...
    """
    # http://www.bloggerpolis.com/2011/05/how-to-check-if-a-process-is-running-using-python/
...
def set_wallpaper(file_loc: str, first_run: bool = True):
    ...
    if desktop_env in ["gnome", "unity", "cinnamon"]:
        ...
        KEY = "picture-uri"
        # Needed for Ubuntu 22 in dark mode. Might be better to set only one or the other,
        # depending on the current theme. In the settings it will say "This background
        # selection only applies to the dark style" even if it's set for both, arguably
        # referring to the selection that you can make on that page. -- @1j01
    ...
    # see http://blog.zx2c4.com/699 for a solution that might work
        ...
        subprocess.Popen(args)
    # I have not been able to make it work on e17.
    ...
    #     subprocess.Popen(args)
        ...
        # https://stackoverflow.com/questions/431205/how-can-i-programatically-change-the-background-in-mac-os-x
        try:
